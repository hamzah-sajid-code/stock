...
class HyperYahooProvider implements DataProvider {
  ...
  async getHistoricalData(ticker: string, range: TimeRange): Promise<StockData> {
    ...
    let attempts = 0;
    const MAX_ATTEMPTS = 15; // Try 15 times before giving up
    ...
    while (attempts < MAX_ATTEMPTS) {
        ...
        try {
            ...
            lastError = e;
            console.warn(`[StockService] Attempt ${attempts}/${MAX_ATTEMPTS} failed for ${ticker}. Retrying...`);
    ...
    console.error(`[StockService] All attempts failed for ${ticker}.`);
    throw new Error(`Failed to load data for ${ticker} after ${MAX_ATTEMPTS} attempts. The proxies may be rate-limited.`);
  }
